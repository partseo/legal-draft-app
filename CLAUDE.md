...
- `cases/{year_client_matter}/` — 사건 단위 작업 폴더 (input/, case_context.json, research/, output/)
- `rules/citation_rules.md` — 조문·판례 인용 표기 + 출처 핀 규칙 (모든 인용에 적용)
- `rules/case_context.schema.json` — canonical 사건컨텍스트 스키마
- `templates/` — docxtpl 템플릿 2종, render_document.py, check_projection.py
...
2. **인용 단일 진입로** — 조문·판례 인용은 `research/legal_analysis.md`에 원문 발췌가
   있는 것만 가능. 즉석 인용 금지. 규칙은 `rules/citation_rules.md`.
3. **모르면 확인필요** — 불확실한 값(이율·관할·기간 등)은 추측하지 않고
   사건컨텍스트 `확인필요`에 적재하고 본문에 `[변호사 확인 필요]` 표기.
4. **검증 게이트** — /verify-citations에서 ❌가 하나라도 있으면 해당 서면은
   "제출 금지" 상태. 수정 후 재검증.
5. **렌더 전 키 검사** — render_document.py 실행 전 반드시
   `python "templates/check_projection.py" <소장|준비서면> <context.json>` 으로 OK 확인.

## 자주 쓰는 명령

- 렌더: `python "templates/render_document.py" <템플릿.docx> <context.json> <출력.docx>`
