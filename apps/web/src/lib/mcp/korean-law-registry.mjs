...
import { searchAdminRule, SearchAdminRuleSchema, getAdminRule, GetAdminRuleSchema, compareAdminRuleOldNew, CompareAdminRuleOldNewSchema } from "korean-law-mcp/tools/admin-rule";
...
import { searchTreaties, searchTreatiesSchema, getTreatyText, getTreatyTextSchema } from "korean-law-mcp/tools/treaties";
...
import { getLegalTermKB, getLegalTermKBSchema, getLegalTermDetail, getLegalTermDetailSchema, getDailyTerm, getDailyTermSchema, getDailyToLegal, getDailyToLegalSchema, getLegalToDaily, getLegalToDailySchema, getTermArticles, getTermArticlesSchema, getRelatedLaws, getRelatedLawsSchema } from "korean-law-mcp/tools/knowledge-base";
import { searchFtcDecisions, searchFtcDecisionsSchema, getFtcDecisionText, getFtcDecisionTextSchema, searchPipcDecisions, searchPipcDecisionsSchema, getPipcDecisionText, getPipcDecisionTextSchema, searchNlrcDecisions, searchNlrcDecisionsSchema, getNlrcDecisionText, getNlrcDecisionTextSchema, searchAcrDecisions, searchAcrDecisionsSchema, getAcrDecisionText, getAcrDecisionTextSchema } from "korean-law-mcp/tools/committee-decisions";
import { searchSchoolRules, searchSchoolRulesSchema, getSchoolRuleText, getSchoolRuleTextSchema, searchPublicCorpRules, searchPublicCorpRulesSchema, getPublicCorpRuleText, getPublicCorpRuleTextSchema, searchPublicInstitutionRules, searchPublicInstitutionRulesSchema, getPublicInstitutionRuleText, getPublicInstitutionRuleTextSchema } from "korean-law-mcp/tools/institutional-rules";
import { searchAppealReviewDecisions, searchAppealReviewDecisionsSchema, getAppealReviewDecisionText, getAppealReviewDecisionTextSchema, searchAcrSpecialAppeals, searchAcrSpecialAppealsSchema, getAcrSpecialAppealText, getAcrSpecialAppealTextSchema } from "korean-law-mcp/tools/special-admin-appeals";
import { getHistoricalLaw, getHistoricalLawSchema, searchHistoricalLaw, searchHistoricalLawSchema } from "korean-law-mcp/tools/historical-law";
import { getLawSystemTree, getLawSystemTreeSchema } from "korean-law-mcp/tools/law-system-tree";
import { getLinkedOrdinances, LinkedOrdinancesSchema, getLinkedOrdinanceArticles, LinkedOrdinanceArticlesSchema, getDelegatedLaws, DelegatedLawsSchema, getLinkedLawsFromOrdinance, LinkedLawsFromOrdinanceSchema } from "korean-law-mcp/tools/law-linkage";
import { analyzeDocument, AnalyzeDocumentSchema } from "korean-law-mcp/tools/document-analysis";
import { verifyCitations, VerifyCitationsSchema } from "korean-law-mcp/tools/verify-citations";
import { impactMap, ImpactMapSchema } from "korean-law-mcp/tools/impact-map";
import { citeCheck, CiteCheckSchema } from "korean-law-mcp/tools/cite-check";
import { applicableLaw, ApplicableLawSchema } from "korean-law-mcp/tools/applicable-law";
// 통합 진입점 (v4.4.0 — 노출 도구 수 축소용)
import { legalResearch, LegalResearchSchema } from "korean-law-mcp/tools/legal-research";
import { legalAnalysis, LegalAnalysisSchema } from "korean-law-mcp/tools/legal-analysis";
// Chain tool imports
import { chainLawSystem, chainLawSystemSchema, chainActionBasis, chainActionBasisSchema, chainDisputePrep, chainDisputePrepSchema, chainAmendmentTrack, chainAmendmentTrackSchema, chainOrdinanceCompare, chainOrdinanceCompareSchema, chainFullResearch, chainFullResearchSchema, chainProcedureDetail, chainProcedureDetailSchema, chainDocumentReview, chainDocumentReviewSchema, } from "korean-law-mcp/tools/chains";
/**
 * 모든 MCP 도구 정의
 */
