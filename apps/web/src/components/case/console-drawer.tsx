import { Check, ChevronDown, CircleAlert, Loader2, Pencil, Plus, Trash2, X } from "lucide-react";
...
/** 실행 콘솔 드로어 — 실행중 모드 (④-b) */
export function RunningConsole({ title, items, footer }: { title: string; items: TimelineItem[]; footer: string }) {
  return (
    <aside className="flex w-[420px] shrink-0 flex-col gap-3 border-l border-neutral-200 bg-white p-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <button className="flex items-center gap-1.5 rounded-md border border-st-block bg-white px-3 py-1.5 text-[13px] font-semibold text-st-block">
...
/** 체크포인트 드로어 — 쟁점 승인 (④-c). 응답 액션 연결은 P2. */
export function CheckpointConsole({ issues }: { issues: CheckpointIssue[] }) {
  return (
    <aside className="flex w-[420px] shrink-0 flex-col gap-3 border-l border-neutral-200 bg-white p-5">
      <div className="flex flex-col gap-3.5 rounded-lg border border-st-action bg-white p-4">
        ...
        <div className="flex flex-col gap-2">
          {issues.map((s) => (
            <div key={s.id} className="flex flex-col gap-2.5 rounded-lg border border-neutral-200 bg-neutral-50 p-3">
              <div className="flex items-center gap-2">
                <span className="rounded bg-app-tint px-[7px] py-0.5 font-mono text-[11px] font-semibold text-app-primary">
                  {s.id}
                </span>
                <span className="flex-1 text-[13px] font-semibold text-neutral-950">{s.title}</span>
                <span className="flex items-center gap-1.5">
                  <Pencil className="size-[15px] text-neutral-500" />
                  <Trash2 className="size-[15px] text-neutral-500" />
                </span>
              </div>
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-medium text-zinc-400">우리 주장</span>
                <span className="text-xs leading-snug text-zinc-600">{s.claim}</span>
              </div>
            </div>
          ))}
        </div>
        <button className="flex items-center justify-center gap-1.5 rounded-md px-3 py-2 text-[13px] font-semibold text-app-primary">
          <Plus className="size-[15px]" />
          쟁점 추가
        </button>
        <button className="flex items-center justify-center gap-2 rounded-md bg-app-primary px-4 py-[11px] text-sm font-semibold text-white">
          <Check className="size-4" />
          {issues.length}개 쟁점 승인하고 계속
        </button>
      </div>
